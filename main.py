...
This script allows the user to run several deep models (and SVM basenes)
...
parser.add_argument('--dataset', type=str, default="PaviaU", choices=dataset_names,
                    help="Dataset to use.")
parser.add_argument('--model', type=str, default='zhouOneDRNN',
                    ...
                    "mou (1D RNN),"
                    "zhouDR (DRConv),"
                    "zhouConstraintRNN (spectral constraint),"
                    "zhouOneDRNN (OneD+spatial),"
                    "zhouFourDRNN,"
                    "zhouEightDRNN,"
                    "zhouRNNC(oneD or FourD + constriant),"
                    "zhou16dRNN"
                    'zhouSpectralAttention,'
                    "mou2"
                    "zhouEightDRNN_kamata_LSTM"
                    "zhouEightDRNN_kamata_Transformer"
                    "zhou_global_local,"
                    "zhouEightDRNN_kamata_singleD"
                    'zhoumultiscanning,'
                    'zhoumultiscanning_Trans'
                    'zhouSSViT'
                    'zhou3dvit'
                    'zhou_single_multi_scanning_Trans'
                    'zhou_slidingLSTM_Trans'
                    'zhou_dual_LSTM_Trans')
...
viz = visdom.Visdom(env=DATASET + ' ' + MODEL, port=8097)
...
for run in range(N_RUNS):
    ...
    display_predictions(color_gt,viz,caption='Ground truth')
    ...
    if MODEL == 'SVM_grid':
        ...
        prediction1 = np.argmax(probabilities, axis=-1)
        prediction2 = np.argmax(probabilities, axis=-1)

    run_results1 = metrics(prediction1, test_gt, ignored_labels=hyperparams['ignored_labels'], n_classes=N_CLASSES)
    run_results2 = metrics(prediction2, test_gt, ignored_labels=hyperparams['ignored_labels'], n_classes=N_CLASSES)

    mask1 = np.zeros(gt.shape, dtype='bool')
    mask2 = np.zeros(gt.shape, dtype='bool')
    for l in IGNORED_LABELS:
        mask1[gt == l] = True  #改这里可以调整结果图 原来是Ture
        mask2[gt == l] = False
    prediction1[mask1] = 0 #改这里可以调整结果图
    prediction2[mask2] = 0

    color_prediction1 = convert_to_color(prediction1)
    color_prediction2= convert_to_color(prediction2)
    display_predictions(color_prediction1, viz, gt=color_prediction2, caption="Prediction vs. all test ground")
    # display_predictions(color_prediction, viz, gt=convert_to_color(test_gt), caption="Prediction vs. test ground truth")
    # plt.show()

    results.append(run_results1)
    show_results(run_results1, viz, label_values=LABEL_VALUES)
    # explore_spectrums(img,gt,LABEL_VALUES,viz,IGNORED_LABELS)
