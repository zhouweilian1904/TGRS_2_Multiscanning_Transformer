...
# from tsne_torch import tsne,pca_torch,x2p_torch
import os
import datetime
import numpy as np
...
from VitNew1_overlapped_patch_2D import overlapped_patch_2D
from VitNew2_overlapped_patch_3D import overlapped_patch_3D
...
from VitNew4_3d_learned_absolute_pos_encoding import one_hot_extend
from VitNew4_3d_learned_absolute_pos_encoding import get_pos_encode
from VitNew4_3d_learned_absolute_pos_encoding import lstm_cell_memory
...
from cvt import CvT as modelCvT
from module import ConvAttention, PreNorm, FeedForward
# from vit_pytorch.recorder import Recorder
# from mlp_mixer_pytorch import MLPMixer
# from kmeans_pytorch import kmeans,kmeans_predict
# from positional_encodings.positional_encodings import PositionalEncoding1D,PositionalEncoding2D,PositionalEncoding3D,PositionalEncodingPermute1D,PositionalEncodingPermute2D,PositionalEncodingPermute3D
# from vit_pytorch import vit as vit_model
...
# import optuna
import torch.optim as optim
from sub_band_semi_IP import subIP
...
def get_model(name, **kwargs):
    ...
    if name == 'nn':
        ...
        kwargs.setdefault('batch_size', 100)
        lr = kwargs.setdefault('lr', 1.0)
        model = Mou2EtAl(n_bands, n_classes)
        # For Adadelta, we need to load the model on GPU before creating the optimizer
        model = model.to(device)
        optimizer = optim.Adam(model.parameters())
        criterion = nn.CrossEntropyLoss(weight=kwargs['weights'])
...
class LiuEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=9):
        ...
        self.conv1 = nn.Conv2d(input_channels, 3, (3, 3))
    ...
    def forward(self, x):
        ...
        x = x_pool1
        x_enc = F.relu(x).reshape(-1, self.features_sizes[2])
        x = x_enc

        x_classif = self.fc_enc(x)
        print('x classify', x_classif.shape)

        #x = F.relu(self.fc1_dec_bn(self.fc1_dec(x) + x_enc))
        x = F.relu(self.fc1_dec(x))
        x = F.relu(self.fc2_dec_bn(self.fc2_dec(x) + x_pool1.reshape(-1, self.features_sizes[1])))
        x = F.relu(self.fc3_dec_bn(self.fc3_dec(x) +x_conv1.reshape(-1, self.features_sizes[0])))
        x = self.fc4_dec(x)
        print('x class, x', x_classif.shape, x.shape)
        return x_classif, x
...
class MouEtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes):
        ...
        self.input_channels = input_channels
        self.gru = nn.GRU(1,50, 1, bidirectional=True)  # TODO: try to change this ? #之前是（1,64,1） pavia——100用100的feature hidden。 indianpine用200
        self.gru2 = nn.GRU(1,1,1)
        self.gru3 = nn.GRU(100,50,1, bidirectional=True)#之前是（64,64，1）  pavia——100用100的feature hidden。 indianpine用200
        self.gru4 = nn.GRU(1,100,1)#之前是（1,64,1） pavia——100用100的feature hidden。 indianpine用200
        self.gru_bn = nn.BatchNorm1d(100 *100)#之前是（64*64）根据 记得根据数据集更改input——channel pavia——100用100的feature hidden。 indianpine用200
        self.tanh = nn.Tanh()
        self.fc = nn.Linear(100 * 100, n_classes) #之前是（64*64,）记得根据数据集更改input——channel pavia——100用100的feature hidden。 indianpine用200
        self.relu = nn.ReLU()
        self.softmax = nn.Softmax()
        self.dropout = nn.Dropout(p=0.5)
    ...
    def forward(self, x):
        ...
        x1_10 = self.gru(x1_10)[0]
        x1_10_l = x1_10[-1] #64
        x1_10_l = torch.unsqueeze(x1_10_l, 0)
        # x1_10_l = x1_10_l.expand(x1_10_l.shape[0],10)
        # plt.subplot(3,3,4)
        # plt.plot(x1_10[:, 1, :].cpu().detach().numpy())
        # plt.plot(x1_10_l[1,:].cpu().detach().numpy())
        # plt.subplot(3,3,7)
        # plt.plot(x1_10_l.cpu().detach().numpy())
        # print('x1_10_l',x1_10_l.shape)
        # print('x1_10', x1_10.shape)
        # print('x1_10[-1]',x1_10[-1].shape)
        x11_20 = self.gru(x11_20)[0]
        x11_20_l = x11_20[-1]
        x11_20_l = torch.unsqueeze(x11_20_l, 0)
        print('x_11_20_l', x11_20_l.shape)
        # x11_20_l = x11_20_l.expand(x11_20_l.shape[0],10)
        # plt.subplot(3,3,5)
        # plt.plot(x11_20[:, 1, :].cpu().detach().numpy())
        x21_30 = self.gru(x21_30)[0]
        x21_30_l = x21_30[-1]
        x21_30_l = torch.unsqueeze(x21_30_l, 0)
        # x21_30_l = x21_30_l.expand(x21_30_l.shape[0],10)
        # plt.subplot(3,3,6)
        # plt.plot(x21_30[:, 1, :].cpu().detach().numpy())
        x31_40 = self.gru(x31_40)[0]
        x31_40_l = x31_40[-1]
        x31_40_l = torch.unsqueeze(x31_40_l,0)
        # x31_40_l = x31_40_l.expand(x31_40_l.shape[0],10)

        x41_50 = self.gru(x41_50)[0]
        x41_50_l = x41_50[-1]
        x41_50_l = torch.unsqueeze(x41_50_l, 0)
        # x41_50_l = x41_50_l.expand(x41_50_l.shape[0],10)

        x51_60 = self.gru(x51_60)[0]
        x51_60_l = x51_60[-1]
        x51_60_l = torch.unsqueeze(x51_60_l, 0)
        # x51_60_l = x51_60_l.expand(x51_60_l.shape[0],10)

        x61_70 = self.gru(x61_70)[0]
        x61_70_l = x61_70[-1]
        x61_70_l = torch.unsqueeze(x61_70_l, 0)
        # x61_70_l = x61_70_l.expand(x61_70_l.shape[0],10)

        x71_80 = self.gru(x71_80)[0]
        x71_80_l = x71_80[-1]
        x71_80_l = torch.unsqueeze(x71_80_l, 0)
        # x71_80_l = x71_80_l.expand(x71_80_l.shape[0],10)

        x81_90 = self.gru(x81_90)[0]
        x81_90_l = x81_90[-1]
        x81_90_l = torch.unsqueeze(x81_90_l,0)
        # x81_90_l = x81_90_l.expand(x81_90_l.shape[0],10)

        x91_100 = self.gru(x91_100)[0]
        x91_100_l = x91_100[-1]
        x91_100_l = torch.unsqueeze(x91_100_l,0)  #size 1,batch,feature
        print('x_91_100_l',x91_100_l.shape)
        # x91_100_l = x91_100_l.expand(x91_100_l.shape[0],10)
        ...
        plt.subplot(5,1,2)
        plt.plot(x_cat_l[1,:].cpu().detach().numpy())
        ...
        plt.subplot(5, 1, 4)
        plt.plot(x_l[1,:].cpu().detach().numpy())
        print('x_l[-1]',x_l.shape)
        x_new = x_l * x_cat_l
        plt.subplot(5, 1, 5)
        plt.plot(x_new[1,:].cpu().detach().numpy())
        ...
        # x_new_l = x[-1]
        # plt.subplot(5,1,4)
        # plt.plot(x_new_l[1, :].cpu().detach().numpy())
        # plt.show()
        # x = self.gru3(x)[0]
        print('5',x.shape)
...
class Mou2EtAl(nn.Module):
    ...
    def __init__(self, input_channels, n_classes):
        # The proposed network model uses a single recurrent layer that adopts our modified GRUs of size 64 with sigmoid gate activation and PRetanh activation functions for hidden representations
        super(Mou2EtAl, self).__init__()
        self.input_channels = input_channels
        self.gru = nn.GRU(1, 64, 64, bidirectional=False)  # TODO: try to change this ?
        # self.conv1D = nn.Conv1d(in_channels=1,out_channels=2048,kernel_size=10,stride=10)
        self.gru_bn = nn.BatchNorm1d( 64 * input_channels)
        # self.gru_bn_trans = nn.BatchNorm1d(2048)
        # self.multiheadattention = nn.MultiheadAttention(embed_dim=1,num_heads=1)
        self.tanh = nn.Tanh()
        self.prelu = nn.PReLU()
        self.fc = nn.Linear(64 * input_channels, n_classes)
        # self.fc_trans = nn.Linear(2048, n_classes)

    def forward(self, x):
        x = x.squeeze()
        x = x.unsqueeze(0)
        # print('x.shape',x.shape) # x is in 1, N, C (1, 100, 204)
        # x = x.permute(1,0,2)
        # x = self.conv1D(x)
        # print('x_conv1d',x.shape)
        # x = x.permute(2,0,1)
        # #Transformer
        # encoder_layer = nn.TransformerEncoderLayer(d_model=2048, nhead=1, dim_feedforward=2048, activation='gelu').to(
        #     device='cuda')
        # transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=1, norm=None).to(device='cuda')
        # x = transformer_encoder(x)
        # print('x_trans.shape', x.shape)  # (25,100,200)
        # x_center = x[10, :, :]
        ...
        #multihead attention
        # query = torch.randn(x.size(2), x.size(1), x.size(0)).to(device='cuda')
        # key = torch.randn(x.size(2), x.size(1), x.size(0)).to(device='cuda')
        # value = torch.randn(x.size(2), x.size(1), x.size(0)).to(device='cuda')
        # # attn_mask = torch.zeros(x.size(1),x.size(0),x.size(0)).to(device='cuda')
        # attn_output, attn_output_weight = self.multiheadattention(query, key, value, need_weights=True)
        # # print('attn_output.size', attn_output.shape)  # (seq_len,batch,feature_dim) (batch,tar_seq_len,sor_seq_len)
        # attn_output = attn_output.permute(1, 2, 0).contiguous() #(batch,feature_dim,seq_len)
        # print('attn_shape',attn_output.shape)


        x = x.permute(1, 2, 0).contiguous() # batch 64 c
        x = x.view(x.size(0), -1)
        x = self.gru_bn(x)
        x = self.tanh(x)
        x = self.fc(x)
...
class zhouOneDRNN(nn.Module):
    ...
    def __init__(self, input_channels, n_classes, patch_size=5):
        ...
        self.gru_2_1 = nn.GRU(input_channels, patch_size ** 2, 1, bidirectional=True)
        self.gru_2_2 = nn.GRU(input_channels, patch_size ** 2, 1, bidirectional=False)
        self.gru_2_3 = nn.GRU(input_channels, patch_size ** 2, 1, bidirectional=False)
        self.gru_2_4 = nn.GRU(input_channels, patch_size ** 2, 1, bidirectional=False)
        self.gru = nn.GRU(patch_size**2, patch_size**2, 1, bidirectional=False, batch_first=False)  # TODO: try to change this ?
        self.gru_bn = nn.BatchNorm1d((patch_size**2) * input_channels)
        self.gru_bn_2 = nn.BatchNorm1d((patch_size**2) * (patch_size**2) * 2)
        self.tanh = nn.Tanh()
        self.relu = nn.ReLU()
        self.softmax = nn.Softmax()
        self.fc = nn.Linear((patch_size**2) * input_channels, n_classes)
        self.fc_2 = nn.Linear((patch_size**2) * (patch_size**2) * 2, n_classes)
    ...
    def forward(self, x):  # 初始是第三方向
        ...
        x8r = x8rt.cuda()
        #下面改变输入值，确定使用哪个方向
        x1r = x1r.permute(1, 0, 2) #(103,16,25)
        x2r = x2r.permute(1, 0, 2)  # (103,16,25)
        x3r = x3r.permute(1, 0, 2)  # (103,16,25)
        x4r = x4r.permute(1, 0, 2)  # (103,16,25)
        x5r = x5r.permute(1, 0, 2)  # (103,16,25)
        x6r = x6r.permute(1, 0, 2)  # (103,16,25)
        x7r = x7r.permute(1, 0, 2)  # (103,16,25)
        x8r = x8r.permute(1, 0, 2)  # (103,16,25)
        # plt.subplot(1, 2, 1)
        ...
        # TK.spines['right'].set_linewidth(bwith)  # 图框右边
        # print(x.shape)
        x1r = x1r.permute(2,1,0) #（25,16,103）
        x2r = x2r.permute(2, 1, 0)  # （25,16,103）
        x3r = x3r.permute(2, 1, 0)  # （25,16,103）
        x4r = x4r.permute(2, 1, 0)  # （25,16,103）
        x5r = x5r.permute(2, 1, 0) #（25,16,103）
        x6r = x6r.permute(2, 1, 0)  # （25,16,103）
        x7r = x7r.permute(2, 1, 0)  # （25,16,103）
        x8r = x8r.permute(2, 1, 0)  # （25,16,103）
        # print('x',x.shape)
        # x = x4t.cuda() #把值给x
        #LSTM
        # h0 = Variable(torch.randn(1, x.shape[1], 49)).cuda()
        # c0 = Variable(torch.randn(1, x.shape[1], 49)).cuda()
        # out,(h_t,c_t) = self.lstm_seq(x,None)
        # print('out',out.shape)
        # print('ht',h_t.shape)
        # print('ct',c_t.shape)
        #导入GRU
        # x = self.gru(x1r+x2r)[0]
        x = self.gru_2_1(x2r)[0]
        # x2 = self.gru_2_2(x2r)[0]
        # x3 = self.gru_2_3(x3r)[0]
        # x4 = self.gru_2_4(x4r)[0]
...
class zhouFourDRNN(nn.Module):
    ...
    def forward(self, x): #初始是第1方向
        ...
        x2r_r = self.gru_2(x2r + x1r_r)[0] #把x1r经过RNN的值，作为x2r的输入
        x2r_r = self.relu(x2r_r)
        # plt.subplot(1,5,2)
        # plt.plot(x2r[:, 0, :].cpu().detach().numpy())
        x3r_r = self.gru_2(x3r + x2r_r)[0]
        x3r_r = self.relu(x3r_r)
        # plt.subplot(1, 5, 3)
        # plt.plot(x3r[:, 0, :].cpu().detach().numpy())
        x4r_r = self.gru_2(x4r + x3r_r)[0]
        x4r_r = self.relu(x4r_r)
        # plt.subplot(1, 5, 4)
        # plt.plot(x4r[:, 0, :].cpu().detach().numpy())
        x5r_r = self.gru_2(x5r + x4r_r)[0]
        x5r_r = self.relu(x5r_r)
        #
        x6r_r = self.gru_2(x6r + x5r_r)[0]
        x6r_r = self.relu(x6r_r)
        #
        x7r_r = self.gru_2(x7r + x6r_r)[0]
        x7r_r = self.relu(x7r_r)
        #
        x8r_r = self.gru_2(x8r + x7r_r)[0]
...
class zhouEightDRNN(nn.Module):
    ...
    def forward(self, x): #初始是第1方向
        ...
        '--------------------------------------------------------------------------------------'
        x1r_out, x1r_hidden = self.gru_3_1(x1r)
        ...
        '--------------------------------------------------------------------------------------'
        x2r_out, x2r_hidden = self.gru_3_2(x2r) #把x1r经过RNN的值，作为x2r的输入
        ...
        '-----------------------------------------------------------------------------------------'
        x3r_out, x3r_hidden = self.gru_3_3(x3r)
        ...
        '----------------------------------------------------------------------------------------'
        x4r_out, x4r_hidden = self.gru_3_4(x4r)
        ...
        '------------------------------------------------------------------------------------------'
        x5r_out,x5r_hidden = self.gru_3_5(x5r)
        ...
        '------------------------------------------------------------------------------------------'
        x6r_out,x6r_hidden = self.gru_3_6(x6r)
        ...
        '---------------------------------------------------------------------------------------------'
        x7r_out,x7r_hidden = self.gru_3_7(x7r)
        ...
        '----------------------------------------------------------------------------------------------'
        x8r_out,x8r_hidden = self.gru_3_8(x8r)
...
class zhouEightDRNN_kamata_LSTM(nn.Module):
    ...
    def forward(self, x): #初始是第1方向
        ...
        x_strategy_1 = self.lstm_stra_1(x_strategy_1)[0]
        ...
        # allscan = x_strategy_1[:, 1, :]
        # allscan = self.linear(allscan)
        # x_strategy_1_laststep_2 = self.linear(x_strategy_1_laststep_2)
        #
        # output_8scan = torch.matmul(allscan, x_strategy_1_laststep_2)
        # output_8scan= self.softmax(output_8scan)
        # output_8scan = output_8scan.unsqueeze(0)

        # plt.plot(output_8scan.transpose(1,0).cpu().detach().numpy(), linewidth=1.5, marker='*')
        # plt.show()
        # sns.heatmap(data=output_8scan.cpu().detach().numpy(), cmap="mako", linewidths=0.1, square=True)
        # plt.show()
        '-------------------------------------------------------------------------------------'
...
def train(net, optimizer, criterion, data_loader, epoch, scheduler=None,
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            # print('data2,', data[:,:,:,9//2, 9//2].squeeze().shape)
            optimizer.zero_grad()
            if supervision == 'full':
                # print('a',data.shape)
                # output_1 = net1(data)
                ...
                print('target',target)
                # a1 = Variable(torch.ones(64), requires_grad=True).cuda()
                # a2 = Variable(torch.ones(64), requires_grad=True).cuda()
                ...
                outs = net(data)
                output, rec = outs
                # loss1 = criterion[0](output, target) + net1.aux_loss_weight * criterion[1](rec, data)
                # loss2 = criterion[0](output, target) + net2.aux_loss_weight * criterion[1](rec, data)
                # loss = loss1 + loss2
        ...
        if val_loader is not None:
            # val_acc_1 = val(net1, val_loader, device=device, supervision=supervision)
            # val_acc_2 = val(net2, val_loader, device=device, supervision=supervision)
        ...
        # Save the weights
        if e % save_epoch == 0:
            # save_model(net1, camel_to_snake(str(net1.__class__.__name__)), data_loader.dataset.name, epoch=e,
            #            metric=abs(metric))
            # save_model(net2, camel_to_snake(str(net2.__class__.__name__)), data_loader.dataset.name, epoch=e,
            #            metric=abs(metric))
